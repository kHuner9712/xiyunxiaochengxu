...
test('production deploy quiesces all writers before the proof backup and rolls back safely before public exposure', () => {
  ...
  const health = readFileSync(resolve(root, 'apps/api/src/health/health.controller.ts'), 'utf8')
  ...
  const liveMigrationIndex = deploy.lastIndexOf('run --rm --no-deps -e MIGRATION_WRITERS_QUIESCED=true api npx prisma migrate deploy')
  const candidateHealthIndex = deploy.indexOf('candidate API is healthy with scheduler maintenance cleared while public Nginx remains stopped')
  const candidateMaintenanceIndex = deploy.indexOf('candidate API still reports migration maintenance before public exposure')
  const candidateSchedulerIndex = deploy.indexOf('candidate scheduler is not ready before public exposure')
  ...
  assert.ok(stopApiIndex < liveMigrationIndex, 'live migration quiesce proof may be passed only after API/background writers are stopped')
  assert.ok(liveMigrationIndex < candidateHealthIndex, 'candidate API health comes after the live schema migration')
  assert.ok(candidateHealthIndex < candidateMaintenanceIndex, 'direct maintenance readiness must be checked after container health')
  assert.ok(candidateMaintenanceIndex < candidateSchedulerIndex, 'maintenance=false must be proven before scheduler readiness')
  assert.ok(candidateSchedulerIndex < publicExposeIndex, 'scheduler readiness must be proven before Nginx is reopened')
  ...
  assert.match(deploy, /MIGRATION_WRITERS_QUIESCED=true/)
  assert.match(deploy, /candidate API maintenance state, scheduler, business route and Nginx configuration pass before public exposure/)

  const writerProofIndex = entrypoint.indexOf('MIGRATION_WRITERS_QUIESCED')
  ...
  assert.ok(writerProofIndex >= 0, 'production migration must require an explicit HTTP-writer quiesce proof')
  assert.ok(pauseMarkerIndex >= 0, 'production migration must publish the shared scheduler pause marker')
  assert.ok(drainInvocationIndex >= 0, 'production migration must drain existing Cron writers')
  assert.ok(migrationExecIndex >= 0, 'production migration must execute only after the drain gate')
  assert.ok(writerProofIndex < pauseMarkerIndex, 'HTTP writer quiesce proof must be checked before publishing migration maintenance')
  ...
  assert.match(entrypoint, /MIGRATION_WRITERS_QUIESCED:-false/)
  assert.match(entrypoint, /拒绝在线手工迁移/)
  ...
  assert.match(health, /if \(schedulerPaused\)/)
  assert.match(health, /result\.status = 'degraded'/)
  assert.match(health, /isHealthy = false/)
