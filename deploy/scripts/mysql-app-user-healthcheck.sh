...
fail() {
  printf 'mysql app-user healthcheck failed: %s\n' "$1" >&2
  exit 1
}
...
mysql -uroot -p"$MYSQL_ROOT_PASSWORD" -NBe 'SELECT 1' >/dev/null 2>&1 \
  || fail 'MySQL root is not ready or DB_ROOT_PASSWORD is invalid'

app_login_ok=false
if [ "$(mysql -h127.0.0.1 -u"$MYSQL_USER" -p"$MYSQL_PASSWORD" "$MYSQL_DATABASE" -NBe 'SELECT 1' 2>/dev/null || true)" = 1 ]; then
  app_login_ok=true
fi

grants_ok=false
if [ "$app_login_ok" = true ]; then
  grants="$(mysql -uroot -p"$MYSQL_ROOT_PASSWORD" -NBe "SHOW GRANTS FOR '${MYSQL_USER}'@'%'" 2>/dev/null || true)"
...
[ "$(mysql -h127.0.0.1 -u"$MYSQL_USER" -p"$MYSQL_PASSWORD" "$MYSQL_DATABASE" -NBe 'SELECT 1' 2>/dev/null || true)" = 1 ] \
  || fail 'application DB user cannot connect to the configured business database'
