...
| 真实支付验证 | [ ] | 需真机沙箱验证 |
| 退款验证 | [ ] | 需真实退款流程验证 |
| 微信审核材料 | [ ] | 需准备隐私协议、类目、资质、食品/奶粉/保健品合规材料 |
| 正式发布 | [ ] | 审核通过后发布 |

| 字段 | 值 |
|------|------|
| 主体公司 | 临沂禧孕文化传媒有限公司 |
| API 域名 | https://api.yunxixiaochengxu.com.cn |
| 后台域名 | https://admin.yunxixiaochengxu.com.cn |
| 服务器 IP | 62.234.69.19 |
| 当前阶段 | 代码冻结完成 → 待预生产实机部署/小程序体验版/真机验收 |
| 版本号 | 1.0.0 |
| commit hash | TODO（本次提交后更新） |
| 负责人 | _待填写_ |
| 预计上线日期 | _待填写_ |
| 是否代码冻结 | 是（v1.0.0-rc.4 tag 待创建） |
| 是否允许新增功能 | 否 |
| 当前结论 | 代码层核心安全项已补强，但 P0 未清零；待 AppID 注入、协议正式化、食品/奶粉/保健品资质、预生产部署、真实支付退款验收完成后，方可提交微信审核 |
...
- [ ] 审核测试账号准备
- [ ] 审核备注准备
- [ ] 食品经营类目添加（如需销售食品/奶粉/保健品）
- [ ] 食品经营许可证或仅销售预包装食品备案凭证上传
- [ ] 保健食品类目添加及资质上传
- [ ] 婴幼儿配方乳粉产品配方注册证明上传
- [ ] 食品安全承诺或合规声明上传
...
| 检查域 | 状态 | 负责人 | 备注 |
|--------|------|--------|------|
| 技术构建 | [x] Go / [ ] No-Go | | test:ci PASS, release:check PASS, v1.0.0-rc.4 tag 待创建 |
...
| 微信审核材料 | [ ] Go / [x] No-Go | | 主体：临沂禧孕文化传媒有限公司；P0：食品/奶粉/保健品资质确认；P0：隐私协议/用户协议正式内容确认；P0：客服电话/微信/退货地址最终确认；P0：微信公众平台类目与资质上传完成；运营字段清单见 docs/14_WECHAT_REVIEW_PACKAGE.md 第7节 |
...
| 2026-05-27 | 文档校准 | commit hash 校准 / release-check 45 PASS / Go/No-Go 真实状态 / 微信审核材料补齐 / v1.0.0-rc.3 tag | PASS | commit hash 校准为 d2944df；RELEASE_CANDIDATE.md release-check 从 44 校准为 45；微信审核材料补齐自提/支付/ICP/资质说明 | d2944df |
| | | | | | |
