...
| 配置项 | 开发环境 | 生产环境 | 文件位置 |
|--------|---------|---------|---------|
| API 基础地址 | `http://127.0.0.1:8080/` | `https://api.xiyun.com/` | `.env.development` / `.env.production` |
| 静态资源地址 | 空（同API域名） | `https://static.xiyun.com/` 或同API域名 | `.env.production` 的 `UNI_APP_STATIC_URL` |
...
2. 编辑 `.env.production`：
```
UNI_APP_REQUEST_URL=https://api.xiyun.com/
UNI_APP_STATIC_URL=https://api.xiyun.com/
```
...
用户上传文件在 `public/upload/` 目录下，访问路径为 `https://api.xiyun.com/upload/`。
...
| 域名类型 | 域名 | 说明 |
|---------|------|------|
| request 合法域名 | `https://api.xiyun.com` | API 请求 |
| uploadFile 合法域名 | `https://api.xiyun.com` | 文件上传 |
| downloadFile 合法域名 | `https://api.xiyun.com` | 文件下载 |
...
| 配置项 | 占位符 | 实际值（部署时填写） |
|--------|--------|-------------------|
| 域名 | `api.xiyun.com` | ____________ |
| 数据库名 | `shopxo_dev` | ____________ |
| 数据库账号 | `xiyun_user` | ____________ |
| 数据库密码 | `<强密码>` | ____________ |
| 后端地址 | `https://api.xiyun.com/` | ____________ |
| 静态资源地址 | `https://api.xiyun.com/` | ____________ |
| 小程序测试号 AppID | `wxda7779770f53e901` | ____________ |
| 小程序测试号 AppSecret | （微信公众平台获取） | ____________ |
| 文件上传路径 | `/www/wwwroot/xiyun-api/public/upload` | ____________ |
| 日志路径 | `/www/wwwroot/xiyun-api/runtime/log` | ____________ |
