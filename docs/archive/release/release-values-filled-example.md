...
| 3 | 服务器域名(request) | — | `https://api.xiyun-example.com` | 微信后台 → 开发管理 → 服务器域名 |
| 4 | 服务器域名(uploadFile) | — | `https://api.xiyun-example.com` | 同上 |
| 5 | 服务器域名(downloadFile) | — | `https://api.xiyun-example.com` | 同上 |
...
| # | 参数名 | 占位符 | 示例值（假值） | 填入位置 |
|---|--------|--------|--------------|---------|
| 1 | API 域名 | `{{API_DOMAIN}}` | `api.xiyun-example.com` | Nginx 配置、前端环境变量、微信后台 |
| 2 | CDN 域名 | `{{CDN_DOMAIN}}` | `cdn.xiyun-example.com` | 前端环境变量 |
| 3 | 部署目录 | `{{DEPLOY_PATH}}` | `/data/www/xiyun` | Nginx root、目录权限脚本 |
| 4 | SSL 证书路径 | `{{SSL_CERT_PATH}}` | `/etc/nginx/ssl` | Nginx ssl_certificate |
| 5 | Nginx server_name | `localhost` | `api.xiyun-example.com` | `deploy/nginx.production.example.conf` |
...
| 3 | 数据库名 | `shopxo` | `xiyun_prod` | 同上 |
| 4 | 数据库用户 | `root` | `xiyun_app` | 同上 |
...
| 2 | 站点名称 | — | `禧孕` | 后台 → 系统设置 → home_site_name |
...
| # | 参数名 | 占位符 | 示例值（假值） | 填入位置 |
|---|--------|--------|--------------|---------|
| 1 | 隐私政策 URL | — | `https://api.xiyun-example.com/agreement/privacy.html` | 后台 → 协议管理 |
| 2 | 注册协议 URL | — | `https://api.xiyun-example.com/agreement/register.html` | 后台 → 协议管理 |
| 3 | 注销协议 URL | — | `https://api.xiyun-example.com/agreement/logout.html` | 后台 → 协议管理 |
...
| # | 参数名 | 占位符 | 示例值（假值） | 填入位置 |
|---|--------|--------|--------------|---------|
| 1 | API 接口地址 | `process.env.UNI_APP_REQUEST_URL` | `https://api.xiyun-example.com/` | HBuilderX 发行时设置 |
| 2 | 静态资源地址 | `process.env.UNI_APP_STATIC_URL` | `https://cdn.xiyun-example.com/` | HBuilderX 发行时设置 |
...
| # | 参数名 | 占位符 | 示例值（假值） | 填入位置 |
|---|--------|--------|--------------|---------|
| 1 | 客服电话 | `{{CONTACT_PHONE}}` | `400-8888-1234` | `docs/sql/xiyun-init-activity-demo.sql` 中 6 处 |
