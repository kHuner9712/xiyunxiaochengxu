...
将项目从「禧孕母婴商城（ShopXO 二开）」整改为「禧孕 V1.0：自营母婴服务小程序」，确保一期功能完全合规。
...
把前端整理成适合当前资质阶段上线的「禧孕 V1.0 自营母婴服务小程序」，突出母婴行业刚需功能，优化用户体验。
...
建立"禧孕一期可上线范围"的统一合规配置，确保前端页面、接口请求、后端接口、后台配置使用同一套 feature flag / qualification gate 逻辑。
