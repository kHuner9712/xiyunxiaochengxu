...
### 15.1 orders 表新增字段 【V1.0 必做】

在现有 orders 表中新增以下字段：

| 字段名 | 类型 | 必填 | 默认值 | 索引 | 说明 |
|--------|------|------|--------|------|------|
| `fulfillment_type` | TINYINT | 否 | 1 | idx_fulfillment_type | 配送方式：1=快递配送，2=到店自提 |
| `pickup_store_id` | BIGINT | 否 | NULL | idx_pickup_store_id | 自提点 ID，关联 pickup_stores.id（fulfillment_type=2 时必填） |
| `pickup_code` | VARCHAR(6) | 否 | NULL | idx_pickup_code | 自提码，6位随机数字（fulfillment_type=2 且支付成功后生成） |
| `picked_up_at` | DATETIME | 否 | NULL | - | 核销时间，门店确认自提的时间 |

**索引详情：**

| 索引名 | 类型 | 字段 |
|--------|------|------|
| `idx_fulfillment_type` | NORMAL | `fulfillment_type` |
| `idx_pickup_store_id` | NORMAL | `pickup_store_id` |
| `idx_pickup_code` | UNIQUE | `pickup_code` |

**业务说明：**

- `fulfillment_type` 默认为 1（快递配送），保持向后兼容
- `pickup_code` 在订单支付成功后生成，使用 `FLOOR(RAND() * 900000 + 100000)` 生成6位随机数字，需校验唯一性
- 自提订单不填写收货地址相关字段（receiver_name/receiver_phone 等），改为填写 pickup_store_id
- 自提有效期：支付成功后7天（后台可配置），超期自动取消并退款

---

### 15.2 pickup_stores 自提点表 【V1.0 必做】
...
|--------|------|------|--------|------|------|
| `id` | BIGINT | 是 | AUTO_INCREMENT | PRIMARY | 主键 |
| `name` | VARCHAR(100) | 是 | - | idx_name | 门店名称 |
| `province` | VARCHAR(20) | 是 | - | - | 省份 |
| `city` | VARCHAR(20) | 是 | - | - | 城市 |
| `district` | VARCHAR(20) | 是 | - | - | 区/县 |
| `detail_address` | VARCHAR(200) | 是 | - | - | 详细地址（不含省市区） |
...
| `business_hours` | VARCHAR(100) | 否 | NULL | - | 营业时间，如"周一至周五 9:00-18:00" |
| `phone` | VARCHAR(20) | 否 | NULL | - | 联系电话 |
| `image` | VARCHAR(500) | 否 | NULL | - | 门店图片 URL |
| `status` | TINYINT | 否 | 1 | idx_status | 状态：1=启用，2=停用 |
| `sort_order` | INT | 否 | 0 | - | 排序序号 |
| `created_at` | DATETIME | 是 | CURRENT_TIMESTAMP | - | 创建时间 |
| `updated_at` | DATETIME | 是 | CURRENT_TIMESTAMP ON UPDATE | - | 更新时间 |
| `deleted_at` | DATETIME | 否 | NULL | - | 软删除时间 |
...
### 15.3 contents 表扩展字段 【V1.0 必做】

在现有 contents 表中新增以下字段：

| 字段名 | 类型 | 必填 | 默认值 | 索引 | 说明 |
|--------|------|------|--------|------|------|
| `content_type` | VARCHAR(20) | 否 | 'article' | idx_content_type | 内容类型：article-文章，video-视频 |
| `video_url` | VARCHAR(500) | 否 | NULL | - | 视频 URL（content_type=video 时必填） |
| `video_cover` | VARCHAR(500) | 否 | NULL | - | 视频封面图 URL |
| `video_duration` | INT | 否 | NULL | - | 视频时长（秒） |
| `placement` | JSON | 否 | NULL | - | 投放位置数组，如 `["activity_tab","knowledge","home_recommend"]` |
| `tags` | JSON | 否 | NULL | - | 标签数组，如 `["种草","评测","辅食"]`，最多5个 |
| `related_product_ids` | JSON | 否 | NULL | - | 关联商品 ID 数组，如 `[1,2,3]`，最多10个 |
| `related_activity_id` | BIGINT | 否 | NULL | - | 关联活动 ID，关联 activities.id |
| `view_count` | INT | 否 | 0 | - | 浏览量/播放量 |

**索引详情：**

| 索引名 | 类型 | 字段 |
|--------|------|------|
| `idx_content_type` | NORMAL | `content_type` |

**业务说明：**

- `content_type` 默认为 'article'，保持向后兼容
- `placement` 使用 JSON 数组存储，支持同一内容投放到多个位置
- `tags` 使用 JSON 数组存储，标签由后台统一管理
- `related_product_ids` 最多关联10个商品，在内容详情页底部展示推荐

---

### 15.4 share_records 表扩展字段 【V1.0 必做】

在现有 share_records 表中新增以下字段：

| 字段名 | 类型 | 必填 | 默认值 | 索引 | 说明 |
|--------|------|------|--------|------|------|
| `campaign_id` | BIGINT | 否 | NULL | idx_campaign_id | 裂变活动 ID，关联 share_campaigns.id |
| `invite_code` | VARCHAR(32) | 否 | NULL | - | 邀请码（分享链接中的 inviterId 参数） |
| `channel` | VARCHAR(20) | 否 | NULL | - | 分享渠道：friend-好友/群，moments-朋友圈海报，qrcode-二维码 |
| `converted_user_id` | BIGINT | 否 | NULL | idx_converted_user_id | 被邀请人用户 ID（分享转化后回填） |
| `converted_at` | DATETIME | 否 | NULL | - | 转化时间（被邀请人注册/首单时间） |

**索引详情：**

| 索引名 | 类型 | 字段 |
|--------|------|------|
| `idx_campaign_id` | NORMAL | `campaign_id` |
| `idx_converted_user_id` | NORMAL | `converted_user_id` |

---

### 15.5 share_campaigns 裂变活动配置表 【V1.0 必做】
...
| `id` | BIGINT | 是 | AUTO_INCREMENT | PRIMARY | 主键 |
| `name` | VARCHAR(100) | 是 | - | - | 活动名称 |
| `enabled` | TINYINT | 否 | 0 | - | 是否启用：1=是，0=否 |
| `inviter_reward_type` | VARCHAR(20) | 是 | 'points' | - | 邀请人奖励类型：points-积分，coupon-优惠券 |
| `inviter_reward_value` | INT | 是 | - | - | 邀请人奖励值（积分数或优惠券模板ID） |
| `inviter_reward_condition` | VARCHAR(20) | 否 | 'first_order' | - | 奖励发放条件：register-注册即发，first_order-首单完成后发放 |
| `invitee_reward_type` | VARCHAR(20) | 否 | NULL | - | 被邀请人奖励类型：points-积分，coupon-优惠券 |
| `invitee_reward_value` | INT | 否 | NULL | - | 被邀请人奖励值 |
| `daily_limit` | INT | 否 | 0 | - | 每日邀请奖励上限，0=不限 |
| `monthly_limit` | INT | 否 | 0 | - | 每月邀请奖励上限，0=不限 |
| `anti_fraud_config` | JSON | 否 | NULL | - | 防刷配置，如 `{"same_device_limit":1,"abnormal_threshold":10,"abnormal_window_hours":24}` |
| `start_time` | DATETIME | 否 | NULL | - | 活动开始时间 |
| `end_time` | DATETIME | 否 | NULL | - | 活动结束时间 |
| `created_at` | DATETIME | 是 | CURRENT_TIMESTAMP | - | 创建时间 |
| `updated_at` | DATETIME | 是 | CURRENT_TIMESTAMP ON UPDATE | - | 更新时间 |
...
- 同一时间只允许一个启用的裂变活动
- `anti_fraud_config` 包含：same_device_limit（同设备限制次数）、abnormal_threshold（异常邀请阈值）、abnormal_window_hours（异常检测时间窗口）

---

### 15.6 user_invite_relations 用户邀请关系表 【V1.0 必做】
...
|--------|------|------|--------|------|------|
| `id` | BIGINT | 是 | AUTO_INCREMENT | PRIMARY | 主键 |
| `inviter_id` | BIGINT | 是 | - | idx_inviter_id | 邀请人用户 ID，关联 users.id |
| `invitee_id` | BIGINT | 是 | - | idx_invitee_id (UNIQUE) | 被邀请人用户 ID，关联 users.id |
| `campaign_id` | BIGINT | 否 | NULL | idx_campaign_id | 裂变活动 ID，关联 share_campaigns.id |
| `bind_time` | DATETIME | 是 | CURRENT_TIMESTAMP | - | 绑定时间 |
| `invitee_first_order_at` | DATETIME | 否 | NULL | - | 被邀请人首单完成时间 |
| `reward_status` | TINYINT | 否 | 0 | idx_reward_status | 奖励状态：0=未发放，1=已发放，2=发放失败，3=风控冻结 |
| `reward_time` | DATETIME | 否 | NULL | - | 奖励发放时间 |
| `device_fingerprint` | VARCHAR(64) | 否 | NULL | - | 被邀请人设备指纹，用于防刷 |
| `created_at` | DATETIME | 是 | CURRENT_TIMESTAMP | - | 创建时间 |
| `updated_at` | DATETIME | 是 | CURRENT_TIMESTAMP ON UPDATE | - | 更新时间 |
...
| 索引名 | 类型 | 字段 |
|--------|------|------|
| `idx_inviter_id` | NORMAL | `inviter_id` |
| `idx_invitee_id` | UNIQUE | `invitee_id` |
| `idx_campaign_id` | NORMAL | `campaign_id` |
| `idx_reward_status` | NORMAL | `reward_status` |

**业务约束：**

- 每个被邀请人只能绑定一个邀请人（invitee_id 唯一索引）
- 邀请关系绑定后不可更改
- 奖励在邀请人首单完成后发放（inviter_reward_condition=first_order 时）

---

### 15.7 customer_service_configs 客服配置表 【V1.1 可选】

> 说明：V1.1 阶段可考虑独立建表，V1.0 阶段建议复用 system_configs 表，以 config_key 前缀 `customer_service_` 存储以下配置项：
> - `customer_service_online_enabled`：在线客服开关
> - `customer_service_phone`：客服电话
> - `customer_service_wechat`：客服微信号
> - `customer_service_qrcode_url`：客服二维码图片 URL
> - `customer_service_work_time`：工作时间
> - `customer_service_offline_message`：非工作时间自动回复语
>
> 如需独立建表，表结构如下：
...
|--------|------|------|--------|------|------|
| `id` | BIGINT | 是 | AUTO_INCREMENT | PRIMARY | 主键 |
| `online_enabled` | TINYINT | 否 | 0 | - | 在线客服开关：1=启用，0=禁用 |
| `phone` | VARCHAR(20) | 否 | NULL | - | 客服电话 |
| `wechat` | VARCHAR(50) | 否 | NULL | - | 客服微信号 |
| `qrcode_url` | VARCHAR(500) | 否 | NULL | - | 客服二维码图片 URL |
| `work_time` | VARCHAR(100) | 否 | NULL | - | 工作时间描述 |
| `offline_message` | VARCHAR(500) | 否 | NULL | - | 非工作时间自动回复语 |
| `created_at` | DATETIME | 是 | CURRENT_TIMESTAMP | - | 创建时间 |
| `updated_at` | DATETIME | 是 | CURRENT_TIMESTAMP ON UPDATE | - | 更新时间 |

**业务说明：**

- 此表仅一条记录（单例模式），更新时直接修改已有记录
- V1.0 阶段建议复用 system_configs 表，V1.1 阶段再迁移为独立表

---

### 15.8 ER 关系补充 【V1.0 必做】

新增表与现有表的关联关系：

```
orders ──1:1──→ pickup_stores（通过 pickup_store_id）
share_records ──N:1──→ share_campaigns（通过 campaign_id）
share_records ──N:1──→ users（通过 converted_user_id）
user_invite_relations ──N:1──→ users（inviter_id → users.id）
user_invite_relations ──N:1──→ users（invitee_id → users.id）
user_invite_relations ──N:1──→ share_campaigns（通过 campaign_id）
contents ──N:1──→ activities（通过 related_activity_id）
```
