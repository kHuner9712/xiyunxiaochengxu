...
| `discount_rate` | DECIMAL(3,2) | 否 | NULL | - | 折扣率，如 0.95 表示 95 折，NULL 表示无折扣 |
| `points_rate` | DECIMAL(3,1) | 否 | 1.0 | - | 积分倍率，如 1.5 表示消费 1 元得 1.5 积分 |
...
| `status` | VARCHAR(20) | 是 | - | idx_status | 状态：pending_review（待审核）/approved（已同意）/rejected（已拒绝）/returned（已退货）/refunded（已退款）/closed（已关闭） |
...
```
pending_review → approved → returned → refunded
       ↓              ↓
   rejected        rejected
       ↓
     closed
