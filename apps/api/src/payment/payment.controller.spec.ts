...
describe('PaymentController callback 异常兜底', () => {
  ...
  it('handleCallback 正常返回微信格式且保持 HTTP 200', async () => {
    mockService.handleCallback.mockResolvedValue({ code: 'SUCCESS', message: '成功' });
    const body = callbackBody();
    const res = createMockResponse();
    const result = await controller.callback(body, {}, { rawBody: Buffer.from('') }, res);
  ...
  it('handleCallback 正常返回 FAIL 时设置 HTTP 500', async () => {
    mockService.handleCallback.mockResolvedValue({ code: 'FAIL', message: '金额不一致' });
    const res = createMockResponse();
    const result = await controller.callback(callbackBody(), {}, { rawBody: Buffer.from('') }, res);
    expect(result).toEqual({ code: 'FAIL', message: '金额不一致' });
    expect(res.status).toHaveBeenCalledWith(500);
  });

  it('handleCallback 抛异常时返回 { code: FAIL } 并设置 HTTP 500', async () => {
  ...
  it('handleCallback 抛 HttpException 时仍返回微信格式并设置 HTTP 500', async () => {
...
describe('PaymentController refundCallback 异常兜底', () => {
  ...
  it('handleRefundCallback 正常返回微信格式且保持 HTTP 200', async () => {
    ...
    const res = createMockResponse();
    const result = await controller.refundCallback(body, {}, { rawBody: Buffer.from('') }, res);
    expect(result).toEqual({ code: 'SUCCESS', message: '成功' });
    expect(res.status).not.toHaveBeenCalled();
  });

  it('handleRefundCallback 正常返回 FAIL 时设置 HTTP 500', async () => {
    mockService.handleRefundCallback.mockResolvedValue({ code: 'FAIL', message: '退款单不存在' });
    const res = createMockResponse();
    const result = await controller.refundCallback(callbackBody(), {}, { rawBody: Buffer.from('') }, res);
    expect(result).toEqual({ code: 'FAIL', message: '退款单不存在' });
    expect(res.status).toHaveBeenCalledWith(500);
  });

  it('handleRefundCallback 抛异常时返回 { code: FAIL } 并设置 HTTP 500', async () => {
  ...
  it('handleRefundCallback 抛 HttpException 时仍返回微信格式并设置 HTTP 500', async () => {
    const { BadRequestException } = await import('@nestjs/common');
