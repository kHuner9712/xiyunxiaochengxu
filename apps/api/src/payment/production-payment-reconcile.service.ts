...
export class ProductionPaymentReconcileService extends PaymentReconcileService {
    ...
    for (const order of timeoutOrders) {
      ...
      try {
        if (payment.status === PAYMENT_STATUS.SUCCESS && payment.transactionId) {
          await this.productionPaymentService.processPaymentSuccess(payment.id, order.id, payment.transactionId, null, order);
          fixed += 1;
          continue;
        }
        ...
        if (tradeState === 'SUCCESS') {
          await this.productionPaymentService.processPaymentSuccess(payment.id, order.id, wechatResult.transaction_id, wechatResult.amount?.total, order);
          fixed += 1;
          continue;
        }
        ...
        const code = this.productionWechatErrorCode(error);
  ...
  private async resolveAfterCloseFailure(order: any, payment: any, closeError: unknown) {
    try {
      ...
      if (tradeState === 'SUCCESS') {
        await this.productionPaymentService.processPaymentSuccess(payment.id, order.id, result.transaction_id, result.amount?.total, order);
      ...
      await this.delayProductionAutoClose(order.id, `close_failed_then_trade_state=${tradeState || 'unknown'}`);
      return { fixed: 0, closable: 0, delayed: 1, failed: 1 };
    } catch (queryError) {
      const code = this.productionWechatErrorCode(queryError);
      if (code === 'ORDER_NOT_EXIST') {
        await this.markPaymentDefinitivelyClosed(payment.id, 'ORDER_NOT_EXIST');
        return { fixed: 0, closable: 1, delayed: 0, failed: 0 };
      }
      this.productionLogger.warn(`微信关单失败且复查状态失败: order=${order.orderNo}, close=${(closeError as Error).message}, query=${(queryError as Error).message}`);
  ...
  private async markPaymentDefinitivelyClosed(paymentId: bigint, reason: string) {
    await this.productionPrisma.orderPayment.updateMany({
      ...
      data: { status: PAYMENT_STATUS.FAILED, rawResponse: { terminalState: reason } },
    });
  }
  ...
  private async delayProductionAutoClose(orderId: bigint, reason: string) {
    ...
    await this.productionPrisma.order.update({ where: { id: orderId }, data: { autoCloseAt: nextCloseAt } });
    await this.productionPrisma.orderLog.create({
      data: { orderId, operatorType: 'system', action: 'auto_close_delay', content: `关单前微信支付状态未形成安全终态，延迟关闭。原因：${reason}` },
    });
    this.productionBusinessEvent.emitWarn('order_auto_close_delayed_for_payment_confirm', 'payment', `订单${orderId.toString()} 关单前支付确认未完成，已延迟关闭`, orderId.toString(), { reason, nextCloseAt: nextCloseAt.toISOString() });
  }

  private productionWechatErrorCode(error: unknown): string | undefined {
