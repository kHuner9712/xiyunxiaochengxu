...
test('core order identifiers remain bigint-safe in delivery and aftersale operations', () => {
  ...
  assert.match(orderDetail, /return String\(route\.params\.id \|\| ''\)\.trim\(\)/)
  assert.match(orderDetail, /orderApi\.getDetail\(orderId\)/)
  assert.match(orderDetail, /String\(nextOrder\.id \|\| ''\) !== orderId/)
  ...
  assert.match(aftersaleDetail, /return String\(route\.params\.id \|\| ''\)\.trim\(\)/)
  assert.match(aftersaleDetail, /aftersaleApi\.getDetail\(aftersaleId\)/)
  assert.match(aftersaleDetail, /String\(nextDetail\.id \|\| ''\) !== aftersaleId/)
...
test('admin aftersale UI matches the structured review API and string status model', () => {
  ...
  assert.match(aftersaleDetail, /const receiverName = returnReceiverName\.value\.trim\(\)/)
  assert.match(aftersaleDetail, /const receiverPhone = returnReceiverPhone\.value\.trim\(\)/)
  assert.match(aftersaleDetail, /const receiverAddress = returnAddress\.value\.trim\(\)/)
  assert.match(aftersaleDetail, /aftersaleApi\.approve\(aftersaleId, \{/)
  assert.match(aftersaleDetail, /refundAmount,/)
  assert.match(aftersaleDetail, /returnReceiverName: receiverName/)
  assert.match(aftersaleDetail, /returnReceiverPhone: receiverPhone/)
  assert.match(aftersaleDetail, /returnAddress: receiverAddress/)
  assert.match(aftersaleDetail, /aftersaleApi\.reject\(aftersaleId, rejectReasonValue\)/)
  assert.match(aftersaleDetail, /aftersaleApi\.refund\(aftersaleId\)/)
  assert.match(aftersaleDetail, /!isCurrentRouteAftersale\(aftersaleId\)/)
...
test('product editing and filtering preserve bigint identifiers', () => {
  ...
  assert.match(productEdit, /return String\(route\.params\.id \|\| ''\)\.trim\(\)/)
  assert.match(productEdit, /productApi\.getDetail\(productIdValue\)/)
  assert.match(productEdit, /String\(d\.id \|\| ''\) !== productIdValue/)
  ...
  assert.match(productEdit, /productApi\.update\(targetProductId, payload\)/)
  assert.doesNotMatch(productEdit, /Number\(route\.params\.id\)/)
  assert.doesNotMatch(productEdit, /id:\s*Number\(d\.id\)/)
  assert.doesNotMatch(productEdit, /productApi\.update\(form\.id, payload\)/)
