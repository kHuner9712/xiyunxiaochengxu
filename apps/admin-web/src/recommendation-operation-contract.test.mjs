...
test('recommendation item management has a real candidate selection flow', () => {
  ...
  assert.match(page, /const recommendationId = currentId\.value/)
  assert.match(page, /\/admin\/recommendation\/candidates\/\$\{recommendationId\}/)
  assert.match(page, /const requestSeq = \+\+candidateLoadSeq/)
  assert.match(page, /requestSeq !== candidateLoadSeq/)
  assert.match(page, /recommendationId !== currentId\.value/)
  assert.match(page, /keyword !== candidateKeyword\.value\.trim\(\)/)
