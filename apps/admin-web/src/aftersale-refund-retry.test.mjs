...
test('refund initiation uses atomic state claims without session locks', () => {
  assert.match(service, /await prisma\.aftersaleOrder\.updateMany\(\{[\s\S]*status: aftersale\.status[\s\S]*status: AftersaleStatus\.pending_refund/)
  assert.match(service, /await prisma\.orderRefund\.updateMany\(\{[\s\S]*id: latestRefundBefore\.id[\s\S]*status: latestRefundBefore\.status[\s\S]*status: REFUND_STATUS\.FAILED/)
  assert.match(service, /claim\.count !== 1/)
  assert.match(service, /退款操作正在处理中，请勿重复提交/)
  assert.match(service, /await this\.paymentService\.createRefund\(/)
  ...
  assert.match(service, /data: \{ status: aftersale\.status \}/)
  assert.match(service, /data: \{ status: originalRetryStatus \}/)
  assert.match(service, /退款请求结果待核实:[\s\S]*请先同步微信退款状态/)

  assert.doesNotMatch(service, /GET_LOCK/)
  assert.doesNotMatch(service, /RELEASE_LOCK/)
  assert.doesNotMatch(service, /runWithRefundLock/)
  assert.doesNotMatch(service, /\$transaction\(async \(tx[\s\S]*paymentService\.createRefund/)
})
